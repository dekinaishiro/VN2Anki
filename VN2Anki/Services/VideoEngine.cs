...
namespace VN2Anki.Services
{
    ...
    public class VideoEngine
    {
        ...
        private IDirect3DDevice GetSharedD3D11Device()
        {
            lock (_deviceLock)
            {
                ...
                _sharedDevice = WinRT.MarshalInterface<IDirect3DDevice>.FromAbi(winrtDevicePtr);
        ...
        private void ResetSharedDevice()
        {
            lock (_deviceLock)
            {
                if (_sharedDevice != null)
                {
                    try { _sharedDevice.Dispose(); } catch { }
                    _sharedDevice = null;
                }
            }
        }
        ...
        private async Task<byte[]?> CaptureWindowWinRTAsync(IntPtr hWnd, int maxWidth)
        {
            ...
            var device = GetSharedD3D11Device();
            ...
            finally
            {
                session?.Dispose();
                framePool?.Dispose();
                // Shared device is kept alive for performance reasons
            }
        }
